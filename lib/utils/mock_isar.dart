// ignore_for_file: public_member_api_docs, invalid_use_of_protected_member
  ...
  class MockIsarCollection<OBJ> extends IsarCollection<OBJ> {
    ...
    // In-memory store: id -> object. Seeded externally for collections that
    // need a default record (e.g. Settings at id=227).
    final Map<int, OBJ> _store = {};

    MockIsarCollection(this._mockIsar);

    /// Pre-populate the store with a known object.
  ...
  class MockIsar extends Isar {
    ...
    /// Pre-seed a default object so that collection<T>().getSync(id) != null.
