...
Watchtower manages several things within a selected storage location, including automatic backups, chapter downloads, and the Local source.
...
If you were using the default locations before, then simply select the existing `Watchtower` folder.
...
**Scoped Storage**'s introduction affects various storage-related functions in **Watchtower**.
